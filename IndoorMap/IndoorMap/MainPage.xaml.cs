...
namespace IndoorMap
{
    ...
    public sealed partial class MainPage : MVVMPage
    {
        ...
        private async void Grid_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            //Whatever the locationSetting is, we should get supportCities first, then get the position
        ...
        { 
            ...
            GeocodingBaiDuAction.FormActionCompleted += async (result, ss) =>
            {
                /*
                ...
                */
                ...
                if (string.Equals(status, "0"))
                {
                    ...
                    var saveCity = StrongTypeViewModel.SupportCities.FirstOrDefault(n => n.name == city);
                    if (saveCity == null)
                    {
                        await new MessageDialog("暂不支持" + city + "，为您切换到默认城市").ShowAsync();
                        StrongTypeViewModel.SelectedCityIndex = 0;
                        return;
                    }
                    StrongTypeViewModel.SelectedCityIndex = StrongTypeViewModel.SupportCities.IndexOf(saveCity);
